...
struct Parse {
  ParseResult go(MLIRContext *ctxt,        // The context, should it be needed
                 ...
                 StringRef parameterName,  // Type parameter name, for error
...
struct Parse<T, enable_if_integral_type<T>> {
  ParseResult go(MLIRContext *ctxt, DialectAsmParser &parser,
                 StringRef parameterName, T &result) {
...
struct Parse<T, enable_if_type<T, bool>> {
  ParseResult go(MLIRContext *ctxt, DialectAsmParser &parser,
                 StringRef parameterName, bool &result) {
    StringRef boolStr;
...
struct Parse<T, enable_if_type<T, StringRef>> {
  ParseResult go(MLIRContext *ctxt, DialectAsmParser &parser,
                 StringRef parameterName, StringRef &result) {
    StringAttr a;
...
struct Parse<T, enable_if_arrayref<T>> {
  ...
  ParseResult go(MLIRContext *ctxt, DialectAsmParser &parser,
                 StringRef parameterName, ArrayRef<inner_t> &result) {
    ...
    result = ArrayRef<inner_t>(parameters);
    return mlir::success();
  }
};
