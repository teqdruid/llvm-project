...
static void emitTypeDefDoc(TypeDef td, raw_ostream &os) {
  ...
  if (td.hasSummary())
    os << "\n" << td.getSummary() << "\n";
  if (td.getMnemonic() &&
      td.getPrinterCode() && *td.getPrinterCode() == "" &&
  ...
  if (members.size() != 0) {
    os << "\n#### Type members:\n\n";
    os << "| Member | MLIR Type | Description |\n"
       << "| :-------: | :-------: | ----------- |\n";
...
static void emitDialectDoc(const Dialect &dialect, ArrayRef<Operator> ops,
                           ArrayRef<Type> types, ArrayRef<TypeDef> typeDefs, raw_ostream &os) {
...
static void emitDialectDoc(const RecordKeeper &recordKeeper, raw_ostream &os) {
  ...
  for (auto dialect: dialectsWithDocs)
    emitDialectDoc(dialect, dialectOps[dialect],
                   dialectTypes[dialect], dialectTypeDefs[dialect], os);
}
